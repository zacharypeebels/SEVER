...
class PostgresSubscriptionStore:
    ...
    def delete_user(self, user_id: str) -> int:
        ...
        with self._db() as conn:
            conn.execute("DELETE FROM bank_connections WHERE user_id = %s", (user_id,))
    ...
    def list_banks(self, user_id: str) -> list[dict]:
        ...
        with self._db() as conn:
            rows = conn.execute(
                "SELECT connection_id, institution, created_at FROM bank_connections WHERE user_id = %s ORDER BY created_at",
                (user_id,),
            ).fetchall()
    ...
    def get_bank(self, user_id: str, connection_id: str) -> Optional[dict]:
        with self._db() as conn:
            r = conn.execute(
                "SELECT * FROM bank_connections WHERE user_id = %s AND connection_id = %s",
    ...
    def bank_tokens(self, user_id: str) -> list[dict]:
        with self._db() as conn:
            rows = conn.execute(
                "SELECT connection_id, access_token_enc FROM bank_connections WHERE user_id = %s", (user_id,)
            ).fetchall()
    ...
    def delete_bank(self, user_id: str, connection_id: str) -> bool:
        with self._db() as conn:
            cur = conn.execute(
                "DELETE FROM bank_connections WHERE user_id = %s AND connection_id = %s", (user_id, connection_id)
            )
